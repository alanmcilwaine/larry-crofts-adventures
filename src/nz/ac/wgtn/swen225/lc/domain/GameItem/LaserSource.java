...
public class LaserSource implements Togglabble {
    ...
    public void updateLasers(GameBoard gameBoard) {
        ...
        while (target != null && target.x() >= 0 &&
                target.x() < gameBoard.getWidth() &&
            ...
            MovableBox box = gameBoard.findBoxAtLocation(target);

            if (box != null) {
                if (box instanceof Mirror m) {
                    // reflect, reset the loop again and run same checks
                    m.reflectLaser(this);
                    continue;
                } else if (box instanceof Crate c) {
                    c.explode(gameBoard.getGameState().boxes());
                } else {
                    break;
                }
            }
     ...
     */
    public Map<Location, String> getLasers() {
        return lasers;
    }
